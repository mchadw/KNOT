...
public class KnotCalculator {
    ...
    private double[][] buildMatrix(KnotGraph graph) {
        List<Crossing> crossings = graph.getCrossings();
        int size = graph.getNumArcs();
        double[][] matrix = new double[size][size];

        for (int i = 0; i < crossings.size(); i++) {
            // getting the Crossing's integer eq. row
            int[] row = crossings.get(i).getEquationRow(size);
           
            // we need it back as a double to do the rest of the math
    ...
    private double calculateDeterminant(double[][] matrix) {
        ...
        for(int i=0; i<n; i++) mat[i] = matrix[i].clone();
        ...
        for (int i = 0; i < n; i++) {//iterate through each row
            ...
            // can't just use zero here (computer's capacity to store very
            //small data points)
            ...
            for (int j = i + 1; j < n; j++) { 
                //iterating through the items in each row, 
                // aka the linear relationship of each crossing
